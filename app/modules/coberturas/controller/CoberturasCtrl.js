...
angular.module('App')
    ...
    .controller("CoberturaController", [
        ...
        function ($scope,
             ...
             * @type {*[]}
             */
            $scope.coberturaTableHeaders = [
                {
                    title: 'Cobertura Nombre',
             ...
             */
            function init() {
                console.info("Iniciando controlador Cobertura");
                ...
                if (isUndefined($sessionStorage.rol_user) || isEmptyString($sessionStorage.rol_user)) {
                    ...
                    //menu sidebar
                    $scope.menus = addActiveClassMenu(JSON.parse($sessionStorage.appMenus), MENU_JCR_SEGUROS_ACTIVE.CODE_COBERTURA_MENU);
             ...
             */
            $scope.getAllCobertura = function () {

                spinnerService.show("spinnerUserList");
                CoberturaService.cleanModeEdit();

                CoberturaService.allCoberturaPaginate($scope.filterCriteria).then(function (data) {
                    $scope.coberturas = data.coberturas;
                    $scope.totalPages = data.totalPages;
                    $scope.totalRecords = data.totalRecords;
                    spinnerService.hide("spinnerUserList");

                    var messageGrowl = CoberturaService.getShowGrowlMessage();

                    if (messageGrowl.isShow) {
                        growl.info(messageGrowl.message);
                        CoberturaService.cleanGrowl();
                    }
